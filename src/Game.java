...
class Game {
    public static class Controller {
        ...
        public Controller(Model model) {
            this.window = new JFrame("Memory Match Gamr"); // Create the window
            ...
            this.window.setResizable(true); // Disable the resizing of the window
    ...
    public static class ButtonActionListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (enabledButtons.size() == 0) { // If all the buttons are disabled
                int choice = JOptionPane.showConfirmDialog(window, "Congratulations, you won! Do you want to restart the game?", "You won!", JOptionPane.YES_NO_OPTION);
                if (choice == JOptionPane.YES_OPTION) {
                    controller.reset(new Model(controller.getModel().getColumns())); // Reset the game if the user chooses to restart
                } else {
                    window.dispose(); // Exit the game if the user chooses not to restart
            ...
            if (model.getTries() == 0) { // If the number of tries is 0
                int choice = JOptionPane.showConfirmDialog(window, "You have no more tries left. Do you want to try again?", "Game Over", JOptionPane.YES_NO_OPTION);
                if (choice == JOptionPane.YES_OPTION) {
                    // Reset the game if the user chooses to try again
                    controller.reset(new Model(controller.getModel().getColumns()));
                } else {
                    // Exit the game if the user chooses to exit
                    window.dispose();
                    SwingUtilities.invokeLater(() -> new SplashScreen());
                }
